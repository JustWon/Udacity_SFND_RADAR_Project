...
for i=(Tr+Gr+1):(Nr/2-(Tr+Gr+1))
    for j=(Td+Gd+1):(Nd-(Td+Gd+1))
        ...
        sum_guard = sum(db2pow(RDM(i-(Gr):i+(Gr),j-(Gd):j+(Gd))),'all');
        sum_train = sum_all - sum_guard;
        N_all = ((Tr+Gr)*2+1) * ((Td+Gd)*2+1);
        N_guard = (Gr*2+1)*(Gd*2+1);
        N_train = N_all-N_guard;
        avg_train = sum_train/N_train;
        threshold = avg_train*offset;
